...
NamedTransactions.prototype.init = function(config) {
  ...
  for(var name in config) {
    ...
    if(!name.match(/^[a-zA-Z0-9\_\-\ ]{1,25}$/)) {
      throw new Error('AppDynamics: invalid name for a named transation');
    }
    ...
    if(typeof(pattern) === 'string') {
      ...
      throw new Error('AppDynamics: named transaction pattern should be string or RegExp');
    }
  ...
  if(count > 10) {
    throw new Error('AppDynamics: max (10) number of named transactions exceeded');
  }
