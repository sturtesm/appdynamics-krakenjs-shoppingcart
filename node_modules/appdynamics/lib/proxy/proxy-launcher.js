...
ProxyLauncher.prototype.startProxy = function(nodeIndex) {
  ...
  var proxyCommDir = proxyTmpDir + '/' + nodeIndex;
  mkdir(proxyCommDir);

  var logsDir=proxyTmpDir+'/logs';
  if(!fs.existsSync(proxyTmpDir)) {
    logsDir = './log/appdynamics'; // Heroku case
  }
  mkdir(logsDir);

  var proxyLogsDir = logsDir + '/' + nodeIndex;
  mkdir(proxyLogsDir);
